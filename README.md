...
/**
 * @section Sensors in UAVs
 *
 * @subsection BMP180_Barometer BMP180 Barometer
 ...
 *
 * @subsection MPU6050_IMU_Module MPU6050 IMU Module
 ...
 *
 * @subsubsection Gyroscope_and_Accelerometer Gyroscope and Accelerometer Model GY-521 (MPU6050)
 ...
 *
 * @subsection GY271_Compass GY-271 Compass
 * A three-axis magnetic field sensor based on the QMC5883L chip, used to measure the intensity of the Earth's magnetic field. It serves as the compass functionality for the UAV, aiding in determining its heading.
 *
 * @subsection Infrared_Sensor Infrared Sensor
 * Utilized to detect the presence of objects near the UAV, commonly used for obstacle avoidance and altitude maintenance.
 *
 * @subsubsection Infrared_Distance_Sensor Infrared Distance Sensor Model KY-032
 *
 * @subsection Ultrasonic_Sensor Ultrasonic Sensor
 * Emits ultrasonic waves and receives their echoes to measure the distance between the UAV and the ground or other objects, used for obstacle avoidance and precise altitude control in low-altitude flight.
 *
 * @subsubsection Ultrasonic_Distance_Detection_Module Ultrasonic Distance Detection Module HY-SRF05
 *
 * @subsection Camera
 * Captures video or still images, which can be used for surveillance, mapping, or image recognition navigation.
 *
 * @section UAV_Sensor_Arrangement Sensor Arrangement in UAVs
 * The construction of UAVs involves the meticulous arrangement of sensors to fulfill functional requirements. 
 * - The BMP180 barometer is typically placed at the center of the UAV to mitigate the effects of airflow. 
 * - The MPU6050 is often closely integrated with the flight controller for real-time adjustments of the UAV's flight attitude. 
 * - The GY-271 compass should be situated in an area with minimal electromagnetic interference to ensure the accuracy of heading indications. 
 * - Infrared and ultrasonic sensors are usually located on the underside or around the UAV for detecting and avoiding obstacles. 
 * - The camera, as per necessity, may be mounted at the front, bottom, or other strategic locations to provide optimal viewing angles.
 */
