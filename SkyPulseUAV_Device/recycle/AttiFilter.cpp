...
void AttiFilterThread::run() {
	...
	while (1) {
		std::cout << "Filter thread is in a loop! Waiting for cv..." << std::endl;
		...
		if (!ready) {
...

			{
				...
				sharedAtti.roll = atan2(2.0f * (q[0] * q[1] + q2 * q3), q[0] * q[0] - q[1] * q[1] - q2 * q2 + q3 * q3) * RAD_TO_DEG;
				sharedAtti.pitch = asin(2.0f * (q[0] * q2 - q3 * q[1])) * RAD_TO_DEG;
				sharedAtti.yaw = atan2(2.0f * (q[0] * q3 + q[1] * q2), q[0] * q[0] + q[1] * q[1] - q2 * q2 + q3 * q3) * RAD_TO_DEG;
